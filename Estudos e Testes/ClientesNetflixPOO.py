...
class AVLTree:
    ...
    def minimum(self, node=None):
        if node is None:
            node = self.root
        if node.left_child:
            return self.minimum(node.left_child)
        else:
            return node.value
    ...
    def simetric_traversal(self, node=None):
        ...
        if node is None:
            node = self.root
        if node.left_child:
            self.simetric_traversal(node.left_child)
        if node.value == node_max:
            print(node.value)
        else:
            print(node.value, end=' ')
        if node.right_child:
            self.simetric_traversal(node.right_child)
        if node is None:
            return 0
ROOT = 'root'
# ========================================================
...
AVL = AVLTree()
cont = True
#   comeco do while para receber as condicoes como paramentro
while cont:
    try:
        entrada = input().split()
    except EOFError:
